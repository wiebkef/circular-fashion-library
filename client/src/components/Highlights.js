...
const Highlights = () => {
  ...
  useEffect(() => {
    axios
      .get("/api/items?status=available&limit=6&page=1")
      .then((res) => {
        setItems(res.data);
      })
      ...
      <div className="container mx-auto grid max-w-2xl grid-cols-1 sm:gap-x-6 gap-y-6 px-4 sm:py-4 sm:px-6 sm:grid-cols-2 lg:grid-cols-3 lg:max-w-7xl lg:px-4">
        {items.map((item) => (
          <Link to={`/shop/${item.id}`} key={item.id}>
            <div className="bg-white shadow-md overflow-hidden rounded-lg max-w-sm mx-auto h-full">
              ...
              <div className="px-4 py-1 flex flex-col items-stretch">
